...
ZTL = [ztl ztl ztl ztl ztl]; % load concentration
...
plot(t1,s1(:,1)*zt*kc./(s1(:,1)+km),'Color',[252 187 161]/255,'LineWidth',2),hold on
plot(t1,s1(:,2)*zt*kc./(s1(:,2)+km),'Color',[240 146 128]/255,'LineWidth',2)
plot(t1,s1(:,3)*zt*kc./(s1(:,3)+km),'Color',[227 105 95]/256,'LineWidth',2)
plot(t1,s1(:,4)*zt*kc./(s1(:,4)+km),'Color',[215 64 62]/255,'LineWidth',2)
plot(t1,s1(:,5)*zt*kc./(s1(:,5)+km),'Color',[203 24 29]/255,'LineWidth',2)
...
a = [0 0 0 0 0]; % sequestration input 
M1 = [1 1 1 1 1]; % positive feedback m1 (target)
M2 = [1 1 1 1 1]; % positive feedback m2 (load)
n = [0 1 1 1 1]; % output gain
ZT = [zt zt zt zt zt] % target concentration
ZTL = [ztl ztl ztl ztl ztl]; % load concentration
